...
getwd()
setwd("./HW")  # need to change this path

############################
#Exercise 1
############################
heartbpchol <- read.csv("heartbpchol.csv", header=TRUE)
str(heartbpchol)
...
psych <- read.csv("psych.csv", header=TRUE)
str(psych)
...
cars <- read.csv("cars_new.csv", header=TRUE)
str(cars)
